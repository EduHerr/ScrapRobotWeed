...
const save = async (data) => {
    ...
    try{
        return new Promise((resolve, reject) => {
            conn.query("CALL SP_NEGATIVE(?, ?, ?, ?)", [1, null, _idWeed, name], (err, result) => {
                if(!err){
                    return result;
                }

                throw 'Error al intentar insertar *Negative: ' + err.message;
            });
        });
    }
    catch(error){
...
const read = async (_idWeed) => {
    try{
        return new Promise((resolve, reject) => {
            conn.query("CALL SP_NEGATIVE(?, ?, ?, ?)", [1, null, _idWeed, null], (err, rows) => {
                if(!err){
